...
class VoyageEmbeddingProvider:
    def __init__(self, config: SnowballConfig):
        self.config = config
        self.model_name = config.embedding.voyage_model
        api_key = os.environ.get(config.embedding.api_key_env)
        if not api_key:
            raise RuntimeError(f"missing embedding API key env: {config.embedding.api_key_env}")
        self.api_key = api_key
        self.api_base_url = config.embedding.api_base_url
    ...
    def embed_batch(self, texts: list[str]) -> list[list[float]]:
        ...
        try:
            ...
            raise RuntimeError(f"voyage embedding request failed: {exc.code} {detail}".strip()) from exc
